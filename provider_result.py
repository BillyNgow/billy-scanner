...
external or derived providers. It does not fetch data, place trades, or
...
class DataType(str, Enum):
      """Types of data fetched from providers."""
      PRICE = "price"
...
class QualityLabel(str, Enum):
      """Data quality levels for provider results."""
      VERIFIED = "VERIFIED"
      ESTIMATED = "ESTIMATED"
      MISSING = "MISSING"
      STALE = "STALE"
...
class ProviderResult:
      """Standard contract for all provider data fetches."""
      provider: Provider
      data_type: DataType
      symbol: str
      ok: bool
    ...
    def to_dict(self) -> dict[str, Any]:
              """Serialize to dict with enums as strings."""
              data = asdict(self)
...
def provider_ok(
      *,
      ...
      """Create a successful VERIFIED provider result."""
...
def provider_estimated(
      *,
      ...
      """Create an ESTIMATED quality provider result."""
      return ProviderResult(
          provider=provider,
          data_type=data_type,
          symbol=symbol,
...
def provider_missing(
      *,
      ...
      """Create a MISSING data provider result."""
      return ProviderResult(
          provider=provider,
          data_type=data_type,
          symbol=symbol,
...
def provider_stale(
      *,
      ...
      """Create a STALE provider result."""
      return ProviderResult(
          provider=provider,
          data_type=data_type,
          symbol=symbol,
...
def provider_rate_limited(
      *,
      ...
      """Create a RATE_LIMITED provider result."""
      return ProviderResult(
          provider=provider,
          data_type=data_type,
          symbol=symbol,
...
def provider_error(
      *,
      ...
      """Create an ERROR provider result."""
      return ProviderResult(
          provider=provider,
          data_type=data_type,
          symbol=symbol,
...
def is_verified(result: ProviderResult) -> bool:
      """Return True if result is VERIFIED, ok, and not stale."""
      return (
          result.ok
          and result.quality == QualityLabel.VERIFIED
...
def is_take_it_eligible_quality(result: ProviderResult) -> bool:
      """Return whether provider data quality can support TAKE_IT.
          ...
          This helper checks only the quality contract. It does not evaluate
              strategy, risk, trend, earnings, liquidity, or broker verification.
                  """
