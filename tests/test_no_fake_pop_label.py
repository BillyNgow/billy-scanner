"""Tests: fake PoP label is replaced with credit_width_proxy.

Definition (Milestone 1, approved):

    credit_width_proxy = credit / width      (clamped to [0.0, 1.0])

    For credit=1.50 and width=5, expected proxy = 0.30 (not a percent).

    Alert text must contain: "Credit/width proxy: 0.30 (not PoP)"

and must NOT contain "PoP:".
...
def test_fmt_trade_emits_credit_width_proxy_and_no_pop_label():
    ...
    assert "Credit/width proxy: 0.30 (not PoP)" in out, out

    assert "PoP:" not in out, out
